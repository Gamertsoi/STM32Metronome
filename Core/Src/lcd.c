...
{	
    ...
    int SR2 = SR * SR; // Short radius squared
    int LR2 = LR * LR; // Long radius squared
    int twoSR2 = 2 * SR2;
    int twoLR2 = 2 * LR2;

    // First half of the ellipse
    int p1 = (int)(LR2 - (SR2 * LR) + (0.25 * SR2));

    while (twoLR2 * x <= twoSR2 * y) {
        // Draw points in all four quadrants
        LCD_DrawDot(usC + x, usP + y, usColor); // Quadrant I
        LCD_DrawDot(usC - x, usP + y, usColor); // Quadrant II
        LCD_DrawDot(usC + x, usP - y, usColor); // Quadrant III
        LCD_DrawDot(usC - x, usP - y, usColor); // Quadrant IV

        if (p1 < 0) {
            p1 += twoLR2 * x + LR2;
        } else {
            y--;
            p1 += twoLR2 * x - twoSR2 * y + LR2;
        }
        x++;
    }
    ...
    int p2 = (int)(SR2 - (LR2 * SR) + (0.25 * LR2));

    while (twoSR2 * y <= twoLR2 * x) {
        // Draw points in all four quadrants
        LCD_DrawDot(usC + x, usP + y, usColor); // Quadrant I
        LCD_DrawDot(usC - x, usP + y, usColor); // Quadrant II
        LCD_DrawDot(usC + x, usP - y, usColor); // Quadrant III
